...
/// <summary>
/// Sprint 15.25 (Lot 14.5, brief §22-§28, §39). Pure formula-level coverage of
/// <see cref="ExecutionSimulator.SimulateCore"/> against hand-computed fixtures. Covers
/// TimeHorizonExecutionTests/BuyExecutionTests/SellExecutionTests/InvalidEntryTests/InvalidDataTests/
/// InsufficientFutureDataTests/NoActionExecutionTests/SimulatedPositionTests from brief §39 in one file,
/// mirroring the consolidation already applied in Lot 14.4's formula test file.
/// </summary>
public sealed class ExecutionSimulatorFormulaTests
{
    ...
    {
        var bars = new List<HistoricalBar> { Bar(0, 100m, 100m, 100m), Bar(5, 106m, 104m, 105m) };
    ...
    {
        var bars = new List<HistoricalBar> { Bar(0, 100m, 100m, 100m), Bar(5, 96m, 94m, 95m) };
    ...
    public void HoldingBars_OneBarHorizon_IsExactlyOne()
    {
        ...
        Assert.Equal(0, position.EntryBarIndex);
        Assert.Equal(1, position.ExitBarIndex);
    }
    ...
    public void HoldingBars_AlwaysEqualsConfiguredHorizon(int horizon)
    {
        ...
        for (int j = 1; j <= horizon; j++)
            bars.Add(Bar(j * 5, 101m, 99m, 100m));
    ...
    // ── §11/§12: THEORETICAL_CLOSE_EXIT convention ──────────────────────────────────────────────────
    ...
    public void ExitPrice_IsExitBarClose_NeverHighLowOrOpen()
    {
        var bars = new List<HistoricalBar>
        {
            ...
            Bar(5, high: 110m, low: 90m, close: 103m, open: 95m)
        };
        ...
        Assert.Equal(bars[1].Timestamp, position.ExitTimestamp);
    ...
    [Fact]
    public void Horizon_ExitIsExactlyAtEntryPlusHorizon_BarBeyondItNeverInfluencesTheResult()
    {
        var bars = new List<HistoricalBar> { Bar(0, 100m, 100m, 100m) };
        for (int j = 1; j <= 10; j++)
            bars.Add(Bar(j * 5, 101m, 99m, 100m));
        bars.Add(Bar(55, high: 1000m, low: 1m, close: 500m)); // bar 11: enormous, must never be read

        SimulatedPosition position = ExecutionSimulator.SimulateCore(
            bars, Candidate(0, DirectionCandidate.BUY_CANDIDATE, 100m), ExecutionConfiguration.Create(10));

        Assert.Equal(PositionStatus.Closed, position.Status);
        Assert.Equal(10, position.ExitBarIndex);
        Assert.Equal(100m, position.ExitPrice); // bar 10's Close, unaffected by bar 11
    }

    // ── §18: insufficient future data ───────────────────────────────────────────────────────────────

    [Fact]
    public void InsufficientFutureData_NeverProducesAClosedPosition_NeverTruncatesTheHorizon()
    {
        var bars = new List<HistoricalBar>
        {
            Bar(0, 100m, 100m, 100m), Bar(5, 100m, 100m, 100m), Bar(10, 100m, 100m, 100m),
            Bar(15, 100m, 100m, 100m), Bar(20, 100m, 100m, 100m) // signal at index 3 = N-2 (N=5)
        };
