...
public sealed class StructuralBreakRule : IDecisionRule
{
    ...
    // See StableRangeRule.cs for the full rationale (Sprint 14 / DEC-01, FUS-02). This rule inverts
    // all four dimensions ((1 - Value) throughout), making it the worst-case shape for the bug: with
    // the raw sentinel (Value=0.0), every missing dimension previously contributed its FULL weight as
    // fabricated "structural instability" evidence.
